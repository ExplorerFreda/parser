...
from supar.modules import (LSTM, MLP, BertEmbedding, BERTEndPoint,
                           Biaffine, CharLSTM, Triaffine)
...
class SimplestBiaffineDependencyModel(nn.Module):
    def __init__(self,
                 n_words,
                 n_feats,
                 n_rels,
        ...
        self.args = Config().update(locals())
        self.feat_embed = BERTEndPoint(model=bert,
                                       n_out=n_feat_embed,
                                       pad_index=feat_pad_index)
        self.n_feat_embed = self.feat_embed.n_out
        self.embed_dropout = IndependentDropout(p=embed_dropout)
        ...
        # the MLP layers
        self.mlp_arc_d = MLP(n_in=self.n_feat_embed, n_out=n_mlp_arc, dropout=mlp_dropout)
        self.mlp_arc_h = MLP(n_in=self.n_feat_embed, n_out=n_mlp_arc, dropout=mlp_dropout)
        self.mlp_rel_d = MLP(n_in=self.n_feat_embed, n_out=n_mlp_rel, dropout=mlp_dropout)
        self.mlp_rel_h = MLP(n_in=self.n_feat_embed, n_out=n_mlp_rel, dropout=mlp_dropout)
    ...
    def forward(self, words, feats):
        ...
        feat_embed = self.feat_embed(feats)
        x = self.embed_dropout(feat_embed)[0]
